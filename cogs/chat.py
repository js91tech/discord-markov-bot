    ...
    async def on_message(self, message: discord.Message):
        ...
        if should_respond:
            ...
            async with message.channel.typing():
                ...
                if settings.get("brain_mode") == "llm" and not use_gif:
                    chat_history = []
                    prev_msg_time = None
                    async for msg in message.channel.history(limit=100):
                        # EFFICIENCY FIX: Skip the message that triggered this, we will format it
                        # properly below so it's not duplicated
                        ...
                        if msg.author == self.bot.user:
                            ...
                            text_part = f"{
                                ...
                                clean_msg_content if clean_msg_content else 'sent an image'}"
                    ...
                    text_part = f"{
                        ...
                        clean_trigger_content if clean_trigger_content else 'sent an image'}"
                    ...
                    # Limit bot's own messages in context to reduce self-echoing
                    bot_msg_seen = 0
                    trimmed_history = []
                    ...
                    for attempt in range(3):
                        ...
                        candidate = await generate_llm_response(dynamic_prompt, chat_history_with_prompt)
                        if not candidate:
                            break
                    ...
                    if not final_content:
                        ...
                        response = self._generate_unique_markov(
                            ...
                            message.content,
                            guild_id,
                            settings["min_response_words"],
                            settings["max_response_words"])
                        if response:
